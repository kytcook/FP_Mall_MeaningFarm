...
<div class="board_pg_area">
 <ul>
  <c:if test="${pageMaker.prev}">
   <a href="recipe_listPage${pageMaker.makeQuery(pageMaker.startPage - 1)}"
   class="layout-pagination-button layout-pagination-prev-page">이전</a>
  </c:if> 
  
  <c:forEach begin="${pageMaker.startPage}" end="${pageMaker.endPage}" var="idx">
   <a href="recipe_listPage${pageMaker.makeQuery(idx)}"
   class="layout-pagination-button layout-pagination-number">${idx}</a>
  </c:forEach>
    
  <c:if test="${pageMaker.next && pageMaker.endPage > 0}">
   <a href="recipe_listPage${pageMaker.makeQuery(pageMaker.endPage + 1)}"    
   class="layout-pagination-button layout-pagination-next-page">다음</a>
  </c:if> 
 </ul>
</div>
